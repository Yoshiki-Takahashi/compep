...
function loadTestcase(file) {
    ...
    const timeoutStr = caseTexts.shift() // Pop first block element
    if (timeoutStr.includes('ms')){
        timeout = parseInt(timeoutStr.split('ms')[0])
    } else {
...
function runTestCase() {
    ...
    console.log(
        ...
        chalk.blue(` Timeout ${timeout} ms\n`)
    )
    _.each(cases, (testCase, i) => {
        let ans;
        try{
            const res = execSync(command, { input: testCase.input, timeout: timeout })
            ans = res.toString('utf8')
        } catch (err) {
